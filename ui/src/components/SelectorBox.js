...
const OptionContent = ({person, active}) => {
  return (
    <>
      ...
      { // only show if the option is active
        !active
        || (
          <span
          ...
          <div className="mt-1 relative">
            <Listbox.Button
              ...
              <span className="flex items-center">
                <span className="ml-3 block truncate">{selectedCustomer.name}</span>
              </span>
                ...
                className="absolute z-10 mt-1 w-full bg-white shadow-lg max-h-56 rounded-md py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none sm:text-sm">
                {
                  people.map((person) => (
                    <Listbox.Option
                      ...
                      value={person}
                    >
                      <OptionContent active={person.id === selectedCustomer.id && person.id} person={person}/>
                    </Listbox.Option>
                  ))
                }
              </Listbox.Options>
